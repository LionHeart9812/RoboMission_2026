#!/usr/bin/env pybricks-micropython
from pybricks.hubs import EV3Brick
from pybricks.ev3devices import (Motor, TouchSensor, ColorSensor,
                                 InfraredSensor, UltrasonicSensor, GyroSensor)
from pybricks.parameters import Port, Stop, Direction, Button, Color
from pybricks.tools import wait, StopWatch, DataLog
from pybricks.robotics import DriveBase
from pybricks.media.ev3dev import SoundFile, ImageFile
from pixycamev3.pixy2 import Pixy2
import time
from linefollower import *
from artefacts import artefactsMain

# This program requires LEGO EV3 MicroPython v2.0 or higher.
# Click "Open user guide" on the EV3 extension tab for more information.

# Create your objects here./Studiengang: Türme erichten
ev3 = EV3Brick()

motor_left = Motor(Port.D, positive_direction=Direction.COUNTERCLOCKWISE)
motor_right = Motor(Port.A, positive_direction=Direction.CLOCKWISE)

lifter = Motor(Port.B, positive_direction=Direction.CLOCKWISE)
grabber = Motor(Port.C, positive_direction=Direction.COUNTERCLOCKWISE)

colorSensor_left = ColorSensor(Port.S1)
colorSensor_right = ColorSensor(Port.S4)

colorReflection_left =  colorSensor_left.reflection()
colorReflection_right = colorSensor_right.reflection()

pixy = Pixy2(port=2, i2c_address=0x54)

seenBlack = 0

ev3.speaker.beep()

#Gestell des Roboters (Durchmessser der Reifen, Radstand)-hahaha
robot = DriveBase(motor_left, motor_right, wheel_diameter=60, axle_track=199)
robot.settings(700, 500, 500, 450)

# Fahren auf Zeit
def driveTime(speeder, timeMS):
    robot.stop()
    stopwatch = StopWatch()
    stopwatch.reset()

    while stopwatch.time() < timeMS:
        motor_left.run(speeder)
        motor_right.run(speeder)

    motor_right.stop()
    motor_left.stop()
    ev3.speaker.beep()
            
# ------------------ Main programm ------------------ #
# Warten bis Mittelknopf gedrückt
ev3.screen.draw_text(x=10, y=10, text="Ready to start")
print("Programm is ready")
ev3.light.on(Color.ORANGE)
while Button.CENTER not in ev3.buttons.pressed():
    pass
ev3.screen.clear()
ev3.light.on(Color.GREEN)
ev3.speaker.beep()
wait(200)

# --- Türme stacken --- #
# grabber.run_time(400, 5000, then=Stop.HOLD, wait=False)
lifter.run_time(-200, 300, then=Stop.HOLD, wait=False)
robot.straight(200)
robot.turn(93)
DriveTillDouble(7, 250)

# In beide Türme reinfahren
robot.drive(200, 0)
wait(900)
robot.stop()
ev3.speaker.beep()
wait(50)

robot.straight(-10)
lifter.run_time(200, 75, then=Stop.COAST, wait=False)
grabber.run_time(-1000, 2750, then=Stop.COAST)

robot.straight(-50)
DriveTillDouble(7, -200)
robot.straight(25)
robot.stop()
robot.turn(93)
robot.straight(400)
DriveTillDouble(7, 400)

robot.turn(70)
DriveTillColor("left", 7, 300)
robot.stop()

# Bro steht bei der Mittellinie
robot.straight(-40)
robot.turn(-65)
robot.straight(225)
robot.stop()
robot.straight(-20)
grabber.run_time(1000, 2350, wait=False, then=Stop.COAST)
wait(2200)

### Rechten Turm mitnehmen
lifter.run_time(-200, 150, then=Stop.HOLD)
robot.straight(-160)
grabber.run_time(-1000, 1900, wait=False, then=Stop.COAST)
robot.turn(91)
robot.straight(80)
robot.turn(-90)
robot.straight(250)

grabber.run_time(1000, 1700, then=Stop.COAST)
lifter.run_time(-150, 1000, then=Stop.HOLD)

robot.straight(-365)
robot.turn(90)

robot.drive(-350, 0)
wait(2000)
robot.stop()
ev3.speaker.beep()
wait(50)

robot.straight(125)
robot.turn(-91)
robot.stop()
wait(50)

DriveTillDouble(9, -250)
robot.stop()
robot.straight(475)
robot.stop()
wait(50)

#Abstellen
grabber.run_time(-1000, 1200, then=Stop.COAST, wait=False)
wait(100)
lifter.run_time(175, 100, then=Stop.COAST, wait=False)
wait(1200)
lifter.run_time(-200, 275, then=Stop.HOLD)
grabber.run_time(-1000, 500, then=Stop.COAST)

#robot.straight(20)
robot.straight(-250)
robot.turn(90)

robot.drive(-350, 0)
wait(1500)
robot.stop()
ev3.speaker.beep()
wait(50)

# Linken Turm nehmen
robot.straight(360)
robot.turn(-90)
lifter.run_time(200, 175, then=Stop.COAST, wait=False)
robot.straight(250)

grabber.run_time(1000, 1650, then=Stop.COAST)
lifter.run_time(-150, 1000, then=Stop.HOLD)

robot.straight(-200)
robot.turn(90)
robot.straight(440)
robot.turn(-90)
robot.straight(180)
robot.stop()

# Abstellen
grabber.run_time(-1000, 1200, then=Stop.COAST, wait=False)
wait(100)
lifter.run_time(175, 100, then=Stop.COAST, wait=False)
wait(1200)
lifter.run_time(-200, 275, then=Stop.HOLD)
grabber.run_time(-1000, 500, then=Stop.COAST)

robot.straight(-200)
robot.turn(45)
robot.straight(-50)

time.sleep(60)

# ----- Aktefakte ----- #
artefactsMain()

# --- Blau + Schwarze Mensch nehmen --- #
lifter.run_time(-200, 500, then=Stop.HOLD, wait=False)
grabber.run_time(-750, 1750, then=Stop.HOLD, wait=False)
robot.straight(315)
robot.turn(-35)
robot.straight(140)
lifter.run_time(200, 500, then=Stop.COAST, wait=False)
robot.straight(-5)
robot.turn(-95)
robot.straight(300)
robot.turn(45)
DriveTillDouble(7, 300)
robot.straight(150)
robot.turn(-75)
robot.stop()
wait(100)
DriveTillColor("left",  7, 250)
robot.stop()
robot.straight(40)
robot.turn(80)
robot.straight(135)
robot.stop()

# Dreckklumpen mitnehmen
LF_StopLine(2.5, 0.3, 25, 300)
robot.stop()

motor_left.run_angle(500, 335)
motor_left.stop()

# Blauen abstellen
robot.straight(-35)
grabber.run_time(750, 2500)
lifter.run_time(-200, 650, then=Stop.HOLD)
robot.straight(240)
robot.stop()

motor_left.run_angle(700, 250)
motor_left.stop()
motor_left.run_angle(-700, 250)
motor_left.stop()

robot.straight(-250)
robot.turn(-35)
robot.straight(75)
robot.stop()

# Schwarzen abstellen
motor_right.run_angle(500, 475)
motor_right.stop()

robot.straight(250)
robot.straight(-150)
lifter.run_time(200, 500, then=Stop.COAST, wait=False)
grabber.run_time(-750, 2700, wait=False)
wait(2500)
robot.straight(-160)

robot.turn(-100)
grabber.run_time(750, 2250, wait=False)
robot.stop()

# --- Rot + Grüne Mensch nehmen --- #
# Zurückfahren
DriveTillColor("right", 19, 300)
wait(15)

robot.straight(1150)
robot.turn(90)
driveTime(-750, 2000)
wait(100)
robot.straight(325)
robot.turn(-90)
DriveTillDouble(9, 300)
wait(15)
robot.straight(75)
robot.stop()
wait(100)
motor_right.run_angle(600, 575)

lifter.run_time(-200, 500, then=Stop.HOLD, wait=False)
robot.straight(60) 
grabber.run_time(-750, 1750, then=Stop.HOLD, wait=False)
robot.turn(-35)
robot.straight(140)
lifter.run_time(200, 500, then=Stop.COAST, wait=False)
robot.straight(-10)
robot.turn(-15)
grabber.run_time(750, 2500, then=Stop.HOLD)
robot.turn(-45)

# Zu Abgabe fahren
robot.straight(475)
robot.stop()

motor_right.run_angle(600, 575)
motor_right.run_angle(-600, 800)
motor_right.stop()
robot.straight(125)
grabber.run_time(-750, 2750, then=Stop.HOLD)
robot.straight(-150)
robot.stop()
lifter.run_time(-200, 500, then=Stop.HOLD, wait=False)
motor_right.run_angle(600, 800)
motor_right.stop()

DriveTillDouble(9, 350)
wait(15)
robot.straight(400)
grabber.run_time(750, 2250, then=Stop.HOLD, wait=False)
robot.straight(-100)